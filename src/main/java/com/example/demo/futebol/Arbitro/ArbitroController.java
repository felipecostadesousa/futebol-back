...
private ArbitroService service;
    ...
    public ResponseEntity<?> update(@RequestBody ArbitroRequest request) {
        ...
        Optional<Arbitro> possivelArbitro = service.findById(request.getId());

        Arbitro arbitro = possivelArbitro.get();
        arbitro.transform(request);
        LOGGER.info("Arbitro encontrado com sucesso (controller): {}");
        service.update(arbitro);
        LOGGER.info("Arbitro atualizado(a) com sucesso: {}");
        return ResponseEntity.ok().header("Custom-Header", "foo").body(arbitro);
    ...
    public ResponseEntity<?> delete(@PathVariable("id") Integer id) {
        ...
        if (possivelObject.isPresent()) {
            ...
            LOGGER.info("Arbitro deletado(a) com sucesso: {}");
            return ResponseEntity.ok().header("Custom-Header", "foo").body(object);
        }else{
            return ResponseEntity.notFound().header("not-found-id", String.valueOf(id)).build();
        }
